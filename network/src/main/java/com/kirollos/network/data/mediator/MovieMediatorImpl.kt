...
class MovieMediatorImpl @Inject constructor(
    private val apiService: ApiService, private val moviesDatabase: MoviesDatabase
) : MoviesMediator {
    override suspend fun getMovies(page: Int): Flow<Resource<Movie>> =
        callbackFlow {
            try {
...
enum class MoviesMediatorPolicy {
    BEGIN, APPEND, REFRESH
}