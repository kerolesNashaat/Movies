...
class SplashViewModel @Inject constructor(
    ...
    private fun doSplashTask() {
        _uiState.update { it.copy(loading = true, success = null, error = null) }
        viewModelScope.launch {
            getConfigurationUseCase.invoke().collectLatest { res ->
                when (res) {
                    is Resource.Failure -> {
                        _uiState.update {
                            it.copy(loading = false, success = null, error = res.error)
                        }
                    }

                    is Resource.Success -> {
                        _uiState.update {
                            it.copy(loading = false, success = res.data, error = null)
                        }
